...
namespace WTFTree
{
    ...
    class CBinaryTree
    {
        ...
        internal TreeNode Copy(TreeNode top)
        {
            return (TreeNode)top.Clone();
        }

        internal void number(TreeNode tree, string num)
        {
            int i = 1;
            tree.Text = num + " " + tree.Text;

            foreach (TreeNode node in tree.Nodes)
            {
                number(node, num + " " + i.ToString("D2"));
                i++;
            }
        }

        internal TreeNode Reverse(TreeNode tree)
        {
            if(tree.Nodes.Count == 0)
            {
                return tree;
            ...
            for(int i = tree.Nodes.Count-1; i >= tree.Nodes.Count-1 / 2; i--)
            {
                tmp = tree.Nodes[i].Text;
                tree.Nodes[i].Text = tree.Nodes[tree.Nodes.Count - i].Text;
                tree.Nodes[tree.Nodes.Count - i].Text = tmp;
            }
            return tree;
        }

        internal TreeNode binary(TreeNode tree)
        {
            ...
            if (tree.Nodes.Count < 3) goto axiba;

            TreeNode tmp;
            for (int i = 2; i < tree.Nodes.Count;)
            {
                tmp = tree.Nodes[i];
                tree.Nodes[i].Remove();
                tree.Nodes[1].Nodes.Add(tmp);
            }
axiba:
            foreach (TreeNode node in tree.Nodes)
            {
